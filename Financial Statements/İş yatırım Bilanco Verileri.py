...
first_df = pd.read_excel("C:/Users/batuh/Desktop/PycharmProjects/Stock Prices Historical Data/Endeks.xlsx")
first_df["BIST TÜM-100"].dropna(inplace=True)

for ticker in first_df["BIST TÜM-100"]:
    hisse = ticker
...
#data_obj = data["value"][x]["itemDescEng"],data["value"][0]["value1"],data["value"][0]["value2"],data["value"][0]["value3"],data["value"][0]["value4"]
    ...
    df[df.columns[1:]] = (df[df.columns[1:]].astype(float)/1000000000)
